...
import {
  ...
  Users, Clock, AlertCircle, Check,
  X
} from "lucide-react"
import { useAuth } from "../hooks/useAuth"

// ─── Mock data (misma estructura que /api/camaras) ───────────────────────────

const MOCK_CAMARAS = [
  { id_camara: 1, nombre: "Sala A", ubicacion: "Humanidades, 2do piso", capacidad: 8,  activo: true,  equipo: ["Espejo unidireccional", "Audio HD", "Grabación en video"] },
  { id_camara: 2, nombre: "Sala B", ubicacion: "Humanidades, 2do piso", capacidad: 12, activo: true,  equipo: ["Espejo unidireccional", "Micrófonos direccionales", "Grabación"] },
  { id_camara: 3, nombre: "Sala C", ubicacion: "Humanidades, 1er piso", capacidad: 4,  activo: false, equipo: ["Espejo unidireccional", "Audio básico"] },
  { id_camara: 4, nombre: "Sala D", ubicacion: "Edificio Central, 3er piso", capacidad: 10, activo: true, equipo: ["Proyector", "Control remoto", "Audio HD"] },
]

// Horarios disponibles mock (bloques de 1 hora)
const HORAS = ["07:00","08:00","09:00","10:00","11:00","12:00","13:00","14:00","15:00","16:00","17:00"]

// Horarios ya ocupados por sala y fecha (mock)
const OCUPADOS = {
  "1-2026-05-22": ["08:00","09:00"],
  "2-2026-05-21": ["10:00","11:00"],
  "4-2026-05-23": ["14:00"],
}

function getOcupados(idCamara, fecha) {
  return OCUPADOS[`${idCamara}-${fecha}`] ?? []
}
...
function StepCamara({ selected, onSelect }) {
  return (
    <motion.div
      key="step1"
              ...
              className={`
                ...
                ${!c.activo
                  ...
                  : isSelected
                    ...
                    : "border-slate-200 bg-white hover:border-blue-300 hover:shadow-sm hover:shadow-blue-500/5"
                }
              `}
            >
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <Users size={11} className="text-slate-300 shrink-0" /> {c.capacidad} personas
                </div>
              </div>

              {c.equipo?.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2.5">
                  {c.equipo.slice(0, 2).map((eq, i) => (
                    <span key={i} className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md">
                      {eq}
                    </span>
                  ))}
                  {c.equipo.length > 2 && (
                    <span className="text-[10px] text-slate-400 px-1">+{c.equipo.length - 2} más</span>
                  )}
                </div>
              )}
            </motion.button>
...
function StepFecha({ camara, fecha, setFecha, horaInicio, setHoraInicio, motivo, setMotivo, errors }) {
  const ocupados    = fecha ? getOcupados(camara.id_camara, fecha) : []
  const horaFin     = horaInicio ? addHour(horaInicio) : null
  const horasValidas = HORAS.slice(0, -1) // última hora no puede ser inicio
...
function StepConfirmar({ camara, fecha, horaInicio, motivo, user }) {
  ...
  const rows = [
    ...
    { label: "Sala",        value: `${camara.nombre} — ${camara.ubicacion}` },
    ...
    >
      ...
      <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
        ...
        <p className="text-xs text-amber-700 leading-relaxed">
          Tu reserva quedará en estado <strong>pendiente</strong> hasta que un administrador la apruebe. Recibirás una notificación cuando cambie su estado.
        </p>
      </div>
    </motion.div>
  )
...
export default function NuevaReserva() {
  ...
  const handleSubmit = async () => {
    setLoading(true)
    // ── Cuando el backend esté listo, reemplaza esto: ─────────────────────
      ...
      <div className="max-w-md mx-auto pt-16 text-center space-y-5">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          <p className="text-sm text-slate-500 mt-1 leading-relaxed">
            Tu solicitud para <strong>{camara?.nombre}</strong> el <strong>{fecha}</strong> a las <strong>{horaInicio}</strong> está pendiente de aprobación.
          </p>
        </motion.div>
        <motion.div
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 min-h-[340px]">
        <AnimatePresence mode="wait">
          {step === 0 && (
            <StepCamara key="s0" selected={camara} onSelect={c => { setCamara(c); setErrors({}) }} />
          )}
          {step === 1 && (
            <StepFecha
              key="s1"
