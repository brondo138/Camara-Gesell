...
const GruposService = {
    async obtenerTodos(filtros = {}) {
        const { id_usuario, id_rol } = filtros;

        if (Number(id_rol) !== 1 && id_usuario) {
            return await GruposRepository.obtenerPorUsuario(id_usuario);
        }
    ...
    async actualizarGrupo(id_grupo, data) {
        ...
        const docenteAnterior = grupoExiste.id_docente_responsable;
        ...
        if (Number(docenteAnterior) !== Number(id_docente_responsable)) {
            await GruposRepository.quitarDocenteAnterior(id_grupo, docenteAnterior);
        }

        await GruposRepository.asignarOActualizarUsuarioGrupo(
            id_grupo,
            id_docente_responsable,
            'Docente'
        );
    ...
    async asignarUsuarioGrupo(id_grupo, data) {
        ...
        if (rol_en_grupo === 'Docente') {
            const docenteAnterior = grupoExiste.id_docente_responsable;

            await GruposRepository.actualizarDocenteResponsable(id_grupo, id_usuario);

            if (Number(docenteAnterior) !== Number(id_usuario)) {
                await GruposRepository.quitarDocenteAnterior(id_grupo, docenteAnterior);
            }

            await GruposRepository.asignarOActualizarUsuarioGrupo(id_grupo, id_usuario, 'Docente');

            return {
                message: 'Docente responsable actualizado correctamente'
            };
        }
